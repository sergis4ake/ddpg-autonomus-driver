...
class ServerState:
    ...
    def __repr__(self):
        ...
        return self.rawout() if self.raw else self.fancyout()
        ...
        for k in sensors:
            if type(self.d.get(k)) is list:  # Handle list type data.
                if k == 'track':  # Nice display for track sensors.
                    ...
                    strout += ' '.join(raw_tsens[:9]) + '_' + raw_tsens[9] + '_' + ' '.join(
                        raw_tsens[10:])
...
class DriverAction:
    ...
    def clip_to_limits(self):
        ...
        if self.d['meta'] not in [0, 1]:
            self.d['meta'] = 0
        if type(self.d['focus']) is not list or min(self.d['focus']) < -180 or max(
                self.d['focus']) > 180:
            self.d['focus'] = 0
