...
def test(sess, env, actor, actor_target, critic, critic_target):
   ...
   for i in range(max_episodes): 
      ...
      for j in range(max_steps):
         ...
         print('| Episode: {:5d} | Step: {:5d}  | Stel: {:2.2f}\t| Accel: {:2.2f}\t| Brake: {:2.2f}\t| Reward: {:6d} | Qmax {:6d} |'.format(i, j, action[0][0], action[0][1], action[0][2], int(reward_total), int(q_max / float(j+1))))

         ###############################################################

         if fin:
            print('| Reward: {:d} | Episode: {:d} | Q-max: {:.4f}'.format(int(reward_total), i, (q_max / float(j))))
            with open(os.path.join(args['resources'], "data") + "/" + args['file'] + "_test.txt", "a") as f:
