...
class ActorNetwork(object):
    ...
    def __init__(self, sess, scope, state_size, action_size, batch_size, lr, n_params):
        ...
        self.critic_gradients = tf.placeholder(dtype=tf.float32, shape=[None, self.action_size],
                                               name='critic_gradients')
        self.gradients = tf.gradients(self.output, self.weights, -self.critic_gradients)
        self.norm_gradients = list(
            map(lambda x: tf.div(x, self.batch_size), self.gradients))  # ¿¿¿NORMALIZED???
    ...
    def train(self, state, critic_gradients):
        ...
        return self.sess.run(self.backprop,
                             feed_dict={self.state: state, self.critic_gradients: critic_gradients})
    ...
    def build_model(self):
        ...
        with tf.variable_scope(self.scope, reuse=tf.AUTO_REUSE):
            state = tf.placeholder(name='actor_state', dtype=tf.float32,
                                   shape=[None, self.state_size])
            fc1 = tf.layers.dense(
                inputs=state, units=H.layer1, activation=tf.nn.relu,
                kernel_initializer=H.kernel_init,
