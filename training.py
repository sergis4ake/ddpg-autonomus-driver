...
def train(sess, env, actor, actor_target, critic, critic_target):
    ...
    if init_episode == 0:
        ...
        saver.restore(
            sess,
            os.path.join(args['resources'], "network") + "/" + args['file'] + "_model"
        )
    ...
    for i in range(init_episode, max_episodes):
        ...
        for j in range(max_steps):
            ...
            # Run step and get data for environment.
            ...
            if (counter % transfer_network_params_every) == 0:
                ...
                if len(buffer) > batch_size:
                    ...
                    critic_q_target = [
                        x if fin_batch[i] else (x + gamma * _critic_q_target[i])
                        for i, x in enumerate(rewards_batch)
                    ]

                    # Train critic network with targets Q values.
                    critic_q_predicted, _ = critic.train(states_t0_batch, actions_batch,
                                                         np.reshape(critic_q_target,
                                                                    (batch_size, 1)))
                ...
                print(
                    '| Buffer {:7} | Episode: {:5d} | Step: {:5d}  | Stel: {:2.2f}\t| Accel: {:2.2f}\t| Brake: {:2.2f}\t| Reward: {:6d} | Qmax: {:6d} | Time: {:2.2E}'.format(
                        len(buffer), i, j, action[0][0], action[0][1], action[0][2],
                        int(reward_total), int(q_max / float(j + 1)), time() - t0))

                ###############################################################

                if fin:
                    print('| Reward: {:d} | Episode: {:d} | Q-max: {:.4f}'.format(int(reward_total),
                                                                                  i, (q_max / float(
                            j))))
                    with open(os.path.join(args['resources'], "data") + "/" + args[
                        'file'] + "_train.txt", "a") as f:
